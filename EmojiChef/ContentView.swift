...
struct MixingGameView: View {
    ...
    var availableRecipes: [Recipe] {
        Recipe.availableRecipes.filter { recipe in
            recipe.ingredients.allSatisfy { gameState.collectedIngredients.contains($0) }
        }
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                ForEach(Recipe.availableRecipes) { recipe in
                    let canMake = recipe.ingredients.allSatisfy { gameState.collectedIngredients.contains($0) }
                    let alreadyMade = gameState.createdRecipes.contains(recipe.name)
                    
                    Button(action: {
                        if canMake && !alreadyMade {
                            gameState.createdRecipes.insert(recipe.name)
                            recipeMessage = "You created \(recipe.emoji) \(recipe.name)!"
                        ...
                        .padding()
                        .background(alreadyMade ?
                                   Color.green.opacity(0.15) :
                                   (canMake ? Color.blue.opacity(0.15) : AppColors.surfaceBackground(for: colorScheme)))
                        .cornerRadius(10)
                    }
                ...
                // Created Recipes
                if !gameState.createdRecipes.isEmpty {
                    VStack(alignment: .leading) {
                        Text("Your Dishes:")
                            .font(.headline)
                            .foregroundColor(.primary)
                        LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 4), spacing: 10) {
                            ForEach(Array(gameState.createdRecipes), id: \.self) { recipeName in
        ...
        .background(Color(.systemBackground))
        .alert("Cooking Result", isPresented: $showingRecipeAlert) {
            Button("Yummy!") {}
        } message: {
            Text(recipeMessage)
        }
